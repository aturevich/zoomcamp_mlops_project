...
def create_reference_data(full_data_path, test_size=0.2, random_state=42):
    ...
    reference_data, _ = train_test_split(
        full_data[features], test_size=1 - test_size, random_state=random_state
    )
    ...
    print(
        f"Reference data saved to '{REFERENCE_DATA_PATH}'. Shape: {reference_data.shape}"
    )
...
def load_reference_data():
    ...
    if not os.path.exists(REFERENCE_DATA_PATH):
        raise FileNotFoundError(
            f"Reference data file not found at {REFERENCE_DATA_PATH}. "
