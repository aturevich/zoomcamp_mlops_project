...
def model_training_pipeline(input_data_path, output_dir):
    ...
    magnitude_model_output_path = os.path.join(
        output_dir, "magnitude_prediction_model.cbm"
    )

    try:
        logger.info("Training magnitude model...")
        magnitude_model_path = train_magnitude_model(
            input_data_path, magnitude_model_output_path
        )
