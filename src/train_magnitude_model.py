...
def prepare_features(data):
    ...
    features = [
        "latitude", "longitude", "day_of_year", "month", "year", "week_of_year",
    ]
    X = data[features]
...
def train_earthquake_model(input_data_path, model_output_path, sample_size=None):
    ...
    try:
        with mlflow.start_run(run_name="earthquake_model_catboost") as run:
            ...
            X_train, X_val, y_mag_train, y_mag_val, y_depth_train, y_depth_val, y_sig_train, y_sig_val = train_test_split(
                X, y_magnitude, y_depth, y_significant, test_size=0.2, random_state=42
            )
            
            # Train magnitude model
            magnitude_model = train_model(X_train, y_mag_train, X_val, y_mag_val, target='magnitude')
            mag_metrics = evaluate_model(magnitude_model, X_val, y_mag_val, task='regression')
            for metric, value in mag_metrics.items():
                mlflow.log_metric(f"magnitude_{metric}", value)
            
            # Train depth model
            depth_model = train_model(X_train, y_depth_train, X_val, y_depth_val, target='depth')
            depth_metrics = evaluate_model(depth_model, X_val, y_depth_val, task='regression')
            for metric, value in depth_metrics.items():
                mlflow.log_metric(f"depth_{metric}", value)
            
            # Train significant event model
            significant_model = train_model(X_train, y_sig_train, X_val, y_sig_val, target='significant')
            sig_metrics = evaluate_model(significant_model, X_val, y_sig_val, task='classification')
            for metric, value in sig_metrics.items():
                mlflow.log_metric(f"significant_{metric}", value)

            # Save models
            os.makedirs(model_output_path, exist_ok=True)
            magnitude_model.save_model(os.path.join(model_output_path, "magnitude_model.cbm"))
            depth_model.save_model(os.path.join(model_output_path, "depth_model.cbm"))
            significant_model.save_model(os.path.join(model_output_path, "significant_model.cbm"))
            ...
            for model_name, model in [("magnitude", magnitude_model), ("depth", depth_model), ("significant", significant_model)]:
                feature_importance = pd.DataFrame({
                    'feature': X.columns,
                    'importance': model.feature_importances_
                }).sort_values('importance', ascending=False)
                
                mlflow.log_table(feature_importance, f"{model_name}_feature_importances.json")
                logger.info(f"Top 5 important features for {model_name}: {', '.join(feature_importance['feature'].head().tolist())}")
...
if __name__ == "__main__":
    ...
    try:
        train_earthquake_model(input_data_path, model_output_path, sample_size=100000)  # Adjust sample size as needed
    except Exception as e:
